...
public abstract class QuadtreeNode extends Renderable {
  ...
  public void render() {
    ...
    if (ContextHolder.getContext().getConfig().isRenderReflection() || ContextHolder.getContext().getConfig()
        .isRenderRefraction()) {
      // render only first two lod's for reflection/refraction
      renderChunk =
          (isleaf && chunkConfig.getLod() == 0) || (!isleaf && chunkConfig.getLod() == 0);// || (!isleaf && lod == 1);
    } else {
      renderChunk = isleaf;
    }
  ...
  private void updateChildNodes() {
    ...
    float distance = (ContextHolder.getContext().getCamera().getPosition().sub(worldPos)).length();
      ...
      for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
          addChild(createChildChunk(getComponents(), quadtreeCache, getWorldTransform(),
              chunkConfig.getLocation().add(new Vec2f(i * chunkConfig.getGap() / 2f, j * chunkConfig.getGap() / 2f)),
              lod, new Vec2f(i, j)));
        }
      }
    }
  ...
  public String getQuadtreeCacheKey() {
    ...
    return String.valueOf(chunkConfig.getLod()) + String.valueOf(chunkConfig.getLocation().getX()) + String.valueOf(
        chunkConfig.getLocation().getY())
        + String.valueOf(chunkConfig.getIndex().getX()) + String.valueOf(chunkConfig.getIndex().getY())
        + worldPos.toString();
  }
  ...
  protected void computeWorldPos() {
    ...
    Vec2f loc = chunkConfig.getLocation().add(chunkConfig.getGap() / 2f).mul(quadtreeConfig.getHorizontalScaling())
        .sub(quadtreeConfig.getHorizontalScaling() / 2f);
  ...
  private float getTerrainHeight(float x, float z) {
    ...
    float h0 = quadtreeConfig.getHeightmapDataBuffer()
        .get(quadtreeConfig.getHeightmap().getMetaData().getWidth() * z0 + x0);
    float h1 = quadtreeConfig.getHeightmapDataBuffer()
        .get(quadtreeConfig.getHeightmap().getMetaData().getWidth() * z0 + x1);
    float h2 = quadtreeConfig.getHeightmapDataBuffer()
        .get(quadtreeConfig.getHeightmap().getMetaData().getWidth() * z1 + x0);
    float h3 = quadtreeConfig.getHeightmapDataBuffer()
        .get(quadtreeConfig.getHeightmap().getMetaData().getWidth() * z1 + x1);
